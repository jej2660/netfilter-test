...
void dump(unsigned char* buf, int size) {
    ...
    if(ipv4->ip_p == 6){
        ...
        if(ntohs(tcph->th_dport) == 80 || ntohs(tcph->th_sport) == 80){
            ...
            if(hostp){
                //printf("\npoint ::%s\n", hostp);
                char *check = (char *)malloc(strlen(domain) + 10);
                strncpy(check,(char *)(hostp+6), strlen(domain));
                //printf("\ncheck :   %s\n", check);
                //printf("\n   result:%d    \n", strcmp(check, domain));
                if(!strcmp(check, domain)){
                    DROP = true;
                }
            }
        }
          ...
          struct nfq_data *nfa, void *data)
{
    ...
    if(DROP) return nfq_set_verdict(qh, id, NF_DROP, 0, NULL);
    return nfq_set_verdict(qh, id, NF_ACCEPT, 0, NULL);
}
void Usage(){
    printf("./netfi <domain>\n");
    printf("./netfi test.gilgil.net\n");
    exit(0);
}
